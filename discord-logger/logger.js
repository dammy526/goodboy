...
console.log("DEBUG WEBSITE_API =", process.env.WEBSITE_API);
...
client.on("ready", async () => {
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    console.log(`伺服器 ${guild.name} 的設定：`, cfg);
...
async function registerErrorHandler(client) {
  ...
  console.log('🛡️ 錯誤處理系統已啟動（Error Handler Enabled）');
}
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === "addep" || commandName === "roleep") {
        ...
        try {
          await axios.post(
            "https://discord.com/api/webhooks/1402892487340593213/lp0pTqx8PrvYrxqQIGKkgZ2JErIcKG9Qhh6JWQsbG59qMwOE2-k4eDNFDFgDX1MDt9Xv", // ⚠️ 換成你的 Webhook URL
            {
              username: "EP 操作通知",
  ...
  let useEmbed = interaction.options.getBoolean('use_embed');

  console.log('修正前參數:', { message, useEmbed });
  ...
  if (useEmbed === null) {
    useEmbed = false; // 預設不使用 embed
  }

  console.log('修正後參數:', { message, useEmbed });

  // 檢查訊息內容
  if (!message || message.trim().length === 0) {
    await interaction.editReply({ 
      content: "❌ 請提供訊息內容。使用方法: `/dm user:@某人 message:你的訊息 use_embed:true/false`" 
    });
    return;
  }
  ...
  try {
    if (useEmbed) {
      const embed = new EmbedBuilder()
        ...
        .setColor('#00A2FF')
        .setFooter({ text: `由 M.E.G. bot 發送` })
        .setTimestamp();
      await target.send({ embeds: [embed] });
    } else {
      await target.send(trimmedMessage);
    }

    await interaction.editReply({ 
      content: `✅ 已成功傳送 DM 給 ${target.tag}\n📝 內容: ${trimmedMessage}` 
    });

  } catch (err) {
    await interaction.editReply({ 
      content: `❌ 無法傳送 DM 給 ${target.tag}（可能關閉私訊）。` 
    });
  }
}
      // 頻道權限指令
      else if (commandName === 'addchannelpermissions') {
      ...
      else if (commandName === 'deletechannelpermissions') {
        const channel = interaction.options.getChannel('channel');
        const target = interaction.options.getMentionable('target');
      ...
      // rank 指令
        ...
        const xp = 0; // 這裡需要從你的資料庫獲取 XP
        const level = 1; // 這裡需要從你的資料庫獲取等級
      ...
      // postevent 指令
        ...
        if (!durationSec) {
          return interaction.reply({ content: '⛔ 時間格式錯誤！請用 1h30min、2h、30min 等格式', flags: MessageFlags.Ephemeral });
        }
        ...
        await interaction.reply({ content: '✅ 活動已成功發布！', flags: MessageFlags.Ephemeral });
      }
      ...
      // checkin 指令
        ...
        if (alreadyChecked) {
          await interaction.reply({ content: `你今天已經簽到了，明天再來吧！`, flags: MessageFlags.Ephemeral });
          return;
        }
      ...
      // addrank 指令
        ...
        if (!rankConfig[rank]) {
          return interaction.reply({ content: '❌ 找不到此階級設定', flags: MessageFlags.Ephemeral });
        }
        ...
        await interaction.reply({ content: `✅ 申請已送出，請等待管理員審核。`, flags: MessageFlags.Ephemeral });
      }
      ...
      // 管理指令
        ...
        if (!target) return interaction.reply({ content: "❌ 找不到成員。", flags: MessageFlags.Ephemeral });
        ...
        try {
          ...
          console.error(err);
          interaction.reply({ content: "❌ 無法踢出，請檢查權限。", flags: MessageFlags.Ephemeral });
        }
      }
        ...
        try {
          ...
          console.error(err);
          interaction.reply({ content: "❌ 禁言失敗，請檢查權限。", flags: MessageFlags.Ephemeral });
        }
      }
        ...
        try {
          ...
          console.error(err);
          interaction.reply({ content: "❌ 解除禁言失敗。", flags: MessageFlags.Ephemeral });
        }
      }
        ...
        try {
          ...
          console.error(err);
          interaction.reply({ content: "❌ 封禁失敗。", flags: MessageFlags.Ephemeral });
        }
      }
        ...
        try {
          ...
          console.error(err);
          interaction.reply({ content: "❌ 解除封禁失敗，可能此人未被封禁。", flags: MessageFlags.Ephemeral });
        }
      }
        ...
        if (!member) return interaction.reply({ content: '找不到指定的成員', flags: MessageFlags.Ephemeral });

        try {
          // 加身分組
          const role = guild.roles.cache.get('1258774311238631475'); // 替換成你的角色 ID
          if (!role) return interaction.reply({ content: '找不到指定的身分組', flags: MessageFlags.Ephemeral });
          await member.roles.add(role);
          ...
          console.error(err);
          interaction.reply({ content: '發生錯誤，無法完成自動申請', flags: MessageFlags.Ephemeral });
        }
      }
    }
    ...
    if (interaction.isButton()) {
      if (interaction.customId.startsWith('complete_')) {
        ...
        if (!rankInfo) return interaction.reply({ content: '❌ 找不到階級設定', flags: MessageFlags.Ephemeral });
...
function updateBotSettings(newSettings) {
    console.log('🔄 收到新設定，更新中...');
    
    // 深度合併設定
    Object.keys(newSettings).forEach(key => {
        if (botSettings[key] && typeof botSettings[key] === 'object') {
            Object.assign(botSettings[key], newSettings[key]);
        } else {
            botSettings[key] = newSettings[key];
        }
    });
...
function applyNewSettings() {
    ...
    if (botSettings.moderation) {
        SPAM_LIMIT = botSettings.moderation.spamLimit || 5;
        WARNING_LIMIT = botSettings.moderation.warningLimit || 3;
        MUTE_DURATION = (botSettings.moderation.muteDuration || 10) * 60 * 1000;
        
        console.log(`🛡️ 防刷頻設定更新: ${SPAM_LIMIT}訊息/${WARNING_LIMIT}警告/${MUTE_DURATION/60000}分鐘禁言`);
    }
    
    // 更新 EP 等級
    if (botSettings.epSystem?.levels) {
        levels.length = 0;
        botSettings.epSystem.levels.forEach(level => {
            levels.push({ name: level.name, ep: level.ep });
        });
        console.log(`💠 EP 等級更新: ${levels.length} 個等級`);
    }
}
...
function getBotStats() {
    return {
        ...
        commands: Object.keys(botSettings.commands || {}).length,
        enabledCommands: Object.values(botSettings.commands || {}).filter(Boolean).length
    };
}

function executeBotCommand(command, options) {
    console.log(`🤖 執行指令: ${command}`, options);
    return { success: true, message: `指令 ${command} 執行完成` };
}
...
const { getGuildConfig, updateGuildConfig } = require("./api.js");

client.on("interactionCreate", async (interaction) => {
    if (!interaction.isChatInputCommand()) return;

    if (interaction.commandName === "設定資訊") {
        const cfg = await getGuildConfig(interaction.guild.id);

        interaction.reply(`伺服器目前 prefix：\`${cfg.prefix}\``);
    }

    if (interaction.commandName === "設定前綴") {
        const prefix = interaction.options.getString("prefix");

        await updateGuildConfig(interaction.guild.id, { prefix });

        interaction.reply(`前綴已更新為：\`${prefix}\``);
    }
});
